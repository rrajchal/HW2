...
function begin_play(){
	var player1_textbox = document.getElementById("player1_id").value;
	var player2_textbox = document.getElementById("player2_id").value;
	
	if (game_started()){
		alert("Already started. Please Reset Play to start again.");
	}
	else{
		if (isEmpty(player1_textbox) || isEmpty(player2_textbox)){
			alert("Two player game, both the fields are mandatory");
		}
		else
		{
			if (!game_started()){
				document.getElementById("player1_id").value = player1_textbox + " (X)";
				document.getElementById("player2_id").value = player2_textbox + " (O)";
			}
			started = true;
...
function play() {
	...
	if (game_started()){
		...
		if (validInput){
			var indexOfInput = table_ids.indexOf(input); 
			if (whose_move()){
				if (board_state[indexOfInput] == -1){
					...
					alert("Already entered, try another cell");
				}
			}
			else {
				if (board_state[indexOfInput] == -1){
...
/*
returns true if three cells in a straight line match
*/
function checkForWin(){
	return ((document.getElementById("A1").innerHTML == document.getElementById("A2").innerHTML && 
